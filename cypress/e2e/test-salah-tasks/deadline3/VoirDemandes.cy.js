describe("Voir Demandes", () => {
  ...
  it("Voir Demandes Vacation", () => {
    ...
    cy.intercept("GET", "http://localhost:3000/vacation", vacations).as(
      "getVacations"
    );
    cy.intercept(
      "GET",
      "http://localhost:3000/etudiant-alumni/111",
      etudiant[0]
    ).as("getEtudiantAlumni1");
    cy.intercept(
      "GET",
      "http://localhost:3000/etudiant-alumni/31",
      etudiant[1]
    ).as("getEtudiantAlumni2");
    cy.visit("/vacations");
    ...
    vacations.map((vacation, index) => {
      cy.get('[data-test="vacation"]')
        .eq(index)
        .within(() => {
          cy.get("h2").should("have.text", `Demande n° ${index + 1}`);
          cy.getByData("nomprenom").should(
            "contain.text",
            etudiant[index].nom + " " + etudiant[index].prenom
          );
          cy.getByData("titre").should("contain.text", vacation.titre);
          cy.getByData("description").should(
            "contain.text",
  ...
  it("Voir Demandes Contrat", () => {
    ...
    cy.intercept("GET", "http://localhost:3000/contrat-expert", contrats).as(
      "getContrat"
    );
    cy.intercept(
      "GET",
